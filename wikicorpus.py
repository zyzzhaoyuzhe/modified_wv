...
# custom helper libarary: lemmatization etc.
import helpers
...
def tokenize(content):
    ...
    return [sent.encode('utf8') for sent in helpers.tokenize(content, lower=True, errors='ignore')
            if 2 <= len(sent) <= 15 and not sent.startswith('_')]
...
def process_article(args):
    ...
    if lemmatize:
        result = helpers.lemmatize(text, allowed_tags=re.compile('(NN|VB|JJ|RB|IN)'), min_length=0, max_length=15, stopwords=frozenset())
    else:
    ...
    def get_texts(self):
        ...
        for group in utils.chunkize(texts, chunksize=15 * self.processes, maxsize=1):
            ...
            for sents, title, pageid in pool.imap(process_article, group):  # chunksize=10):
                ...
                if num_words < ARTICLE_MIN_WORDS or any(title.startswith(ignore + ':') for ignore in IGNORED_NAMESPACES):
                    continue
                articles += 1
                positions += num_words
                if self.metadata:
                    yield (sents, (pageid, title))
                else:
                    yield sents
        pool.terminate()
